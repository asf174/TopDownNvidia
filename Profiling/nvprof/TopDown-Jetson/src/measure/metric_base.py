...
class MetricBase(object):
    ...
    def __init__(self, name : str, description : str, metrics : str, events : str, metrics_desc : str, events_desc : str):
        ...
        Params:
            ...
            metrics             : str   ;   string with the metrics of analy
        
            events              : str   ;   dictionary with metric name as key, 
                                            and value of event as value.
        ...
        if metrics != "":
            self._metrics : dict = dict.fromkeys(metrics.split(","))
            self._metrics_desc : dict = dict.fromkeys(metrics_desc.split(","))

        if events != "":
            self._events : dict = dict.fromkeys(events.split(","))
            self._events_desc : dict = dict.fromkeys(events_desc.split(","))
    ...
    def set_event_description(self, event_name : str, new_description : str) -> bool:
        ...
        if not (event_name in self._event):
            return False
        self._events_desc[event_name] = new_value
        return True
    pass
